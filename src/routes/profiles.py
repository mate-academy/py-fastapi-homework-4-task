...
def profile(
    ...
    Args:
        ...
        profile_form: Form data containing profile information
        jwt_manager: JWT authentication manager
        storage: S3 storage client for avatar upload
        db: Database session

    Returns:
        ProfileResponseSchema: Created profile information

    Raises:
        HTTPException: For various error conditions (401, 403, 400, 422, 500)
    """
    # Verify JWT token
    try:
    ...
    # Create profile and upload avatar
    try:
        # Upload avatar to S3
    ...
    # Return profile with full avatar URL
