...
def profile_creation(
    ...
    try:
        ...
        decoded_token = manager.decode_access_token(access_token)
    except BaseSecurityError:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Could not validate credentials"
        )
    user_id_token = decoded_token.get("user_id")

    user = db.query(UserModel).filter(UserModel.id == user_id_token).first()

    if user_id != user_id_token or user.group.name != UserGroupEnum.ADMIN:
        raise HTTPException(
