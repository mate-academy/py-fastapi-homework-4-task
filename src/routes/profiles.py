...
from exceptions import BaseSecurityError, InvalidTokenError, S3ConnectionError, S3FileUploadError, TokenExpiredError
...
@router.post(
    ...
    responses={
        ...
        status.HTTP_403_FORBIDDEN: {
            ...
            "content": {
                "application/json": {"example": {"detail": "You don't have permission to edit this profile."}}
            },
        },
                ...
                "application/jsom": {
                    "examples": {
                        ...
                        "invalid_birth_date": {
                            ...
                            "value": {"detail": "You must be at least 18 years old to register."},
                        },
                        "invalid_image": {
                ...
                "application/json": {
                    "examples": {
                        "storage_connection": {
                            ...
                            "value": {"detail": "Failed to connect to storage service."},
                        },
                        "upload_error": {
                            "summary": "File Upload Error",
                            "value": {"detail": "Failed to upload avatar. Please try again later."},
                        },
                    }
                }
            },
...
def create_profile(
    ...
    This endpoint handles the creation of a user profile with validation and avatar upload.

    Args:
        request (Request): The FastAPI request object.
        user_id (int): The ID of the user for whom to create the profile.
        profile_form (ProfileRequestForm): The profile data including personal information and avatar.
        db (Session): The database session.
        token (str): The JWT token from the Authorization header.
        storage (S3StorageInterface): The S3 storage client for avatar upload.
        jwt_manager (JWTAuthManagerInterface): The JWT authentication manager.

    Returns:
        dict: The created profile data with avatar URL.

    Raises:
        HTTPException:
            - 400: If profile already exists.
            - 401: If authentication fails or user not found.
            - 403: If user doesn't have permission.
            - 422: If validation fails.
            - 500: If avatar upload fails or storage connection issues.
    """
    try:
        get_token(request)
    ...
    existing_user = db.query(UserModel).filter_by(id=token_data.get("user_id"), is_active=True).first()
