...
def test_registration(
    e2e_client, reset_db_once_for_e2e, settings, seed_user_groups
):
    """
...
def test_account_activation(e2e_client, settings, db_session):
    ...
    activated_user = (
        db_session.query(UserModel).filter_by(email=user_email).first()
    )
...
def test_user_login(e2e_client, db_session):
    ...
    ), "Access token is missing in response!"
    assert (
    ...
    stored_token = (
        db_session.query(RefreshTokenModel)
...
def test_request_password_reset(e2e_client, db_session, settings):
    ...
    assert (
        link_element is not None
    ), "Reset link element with id 'link' not found!"
...
def test_reset_password(e2e_client, db_session, settings):
    ...
    assert (
        deleted_token is None
    ), "Password reset token was not deleted after use!"
    ...
    assert (
        link_element is not None
    ), "Login link element with id 'link' not found!"
    login_url = link_element["href"]
    assert validate_url(login_url), f"The URL '{login_url}' is not valid!"
