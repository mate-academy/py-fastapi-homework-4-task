...
def test_register_user_success(client, db_session, seed_user_groups):
    ...
    created_user = (
        db_session.query(UserModel).filter_by(email=payload["email"]).first()
    )
...
def test_activate_user_with_expired_token(
...
def test_activate_user_with_deleted_token(
    client, db_session, seed_user_groups
):
    """
...
def test_request_password_reset_token_success(
    client, db_session, seed_user_groups
):
    """
