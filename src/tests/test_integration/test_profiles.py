...
def test_create_user_profile_with_fake_s3(
    ...
    assert (
        s3_storage_fake.get_file_url(avatar_key)
        == f"http://fake-s3.local/{avatar_key}"
    )
    ...
    assert (
        profile_in_db.info == "This is a test profile."
    ), "Profile info is incorrect!"
    assert (
        profile_in_db.avatar == avatar_key
    ), "Avatar key in database does not match!"
...
def test_admin_creates_user_profile(
    ...
    profile_in_db = (
        db_session.query(UserProfileModel)
...
def test_cannot_create_profile_twice(
    ...
    ), f"Expected 201, got {response1.status_code}"
    ...
    assert (
        profiles_count == 1
    ), f"Expected only one profile, but found {profiles_count}"
...
def test_profile_creation_fails_on_s3_upload_error(
    ...
    assert (
        profile_in_db is None
    ), "Profile should not be created when S3 upload fails!"
