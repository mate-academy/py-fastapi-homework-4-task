...
class CSVDatabaseSeeder:
    ...
    def _get_or_create_bulk(self, model, items: list, unique_field: str):
        ...
        existing_dict = {
            getattr(item, unique_field): item for item in existing
        }
    ...
    def seed(self):
        try:
            ...
            country_map = self._get_or_create_bulk(
                CountryModel, countries, "code"
            )
            genre_map = self._get_or_create_bulk(
                GenreModel, list(genres), "name"
            )
            actor_map = self._get_or_create_bulk(
